        ...
        conversations: [
            {
                ...
                unread: true,
                messages: [
                    {text: 'Hey! Is the MacBook still available?', sender: 'received', time: '2:30 PM'},
                    {text: 'Yes, it is! Would you like to meet up?', sender: 'sent', time: '2:45 PM'},
                    {text: 'Is the MacBook still available?', sender: 'received', time: '3:00 PM'}
                ]
            },
            {
                id: 'chat2', 
                ...
                unread: false,
                messages: [
                    {text: 'Thanks for the quick delivery!', sender: 'received', time: 'Yesterday'},
                    {text: 'You\'re welcome! Hope you like it!', sender: 'sent', time: 'Yesterday'}
                ]
            }
        ],
        activeChat: null
...
const utils = {
    ...
    showNotification(message, type = 'success') {
        ...
        const colors = {
            success: { bg: 'rgba(0, 255, 136, 0.1)', border: 'rgba(0, 255, 136, 0.3)', color: '#00FF88', icon: '✅' },
            error: { bg: 'rgba(255, 84, 89, 0.1)', border: 'rgba(255, 84, 89, 0.3)', color: '#FF5459', icon: '❌' },
            info: { bg: 'rgba(0, 229, 255, 0.1)', border: 'rgba(0, 229, 255, 0.3)', color: '#00E5FF', icon: 'ℹ️' },
    ...
    startLoading() {
        ...
        const loadingInterval = setInterval(() => {
            ...
            if (messageIndex < messages.length - 1 && progress > (messageIndex + 1) * 25) {
                ...
                setTimeout(() => {
                    this.loadingText.style.animation = 'textPulse 1.5s ease-in-out infinite';
                }, 50);
            }
...
class Marketplace {
    ...
    loadData() {
        ...
        const savedProfile = utils.loadFromStorage('user_profile', AppState.userProfile);
    ...
    bindEvents() {
        ...
        [categoryFilter, conditionFilter, hostelFilter].forEach(filter => {
            if (filter) {
                ...
                // Force dropdown to be interactive
    ...
    filterItems() {
        AppState.filteredItems = AppState.originalItems.filter(item => {
            ...
            const matchesCondition = !AppState.filters.condition || 
                item.condition === AppState.filters.condition;

            const matchesHostel = !AppState.filters.hostel || 
                item.hostel === AppState.filters.hostel;
    ...
    createItemCard(item, index) {
        ...
        const savings = item.originalPrice ? utils.calculateSavings(item.originalPrice, item.price) : null;
        ...
        card.innerHTML = `
            ...
            <div class="item-prices">
                ...
                ${item.originalPrice ? `
                    <div class="item-original-price">${utils.formatPrice(item.originalPrice)}</div>
            ...
            <div class="item-details">
                ...
                ${item.clothingChecklist ? `<span class="item-tag">Quality: ${item.clothingChecklist.quality}</span>` : ''}
            </div>
            <p class="item-description">${item.description}</p>
            <div class="item-actions">
                <button class="btn btn--primary btn--sm contact-btn" data-id="${item.id}">
                    Contact Seller
                </button>
                <button class="heart-btn ${isHearted ? 'hearted' : ''}" data-id="${item.id}" title="Heart this item">
                    ${isHearted ? '❤️' : '🤍'}
                </button>
                ${isUserItem ? `
            ...
            ${item.hearts > 0 ? `<div class="heart-count">${item.hearts} hearts</div>` : ''}
        `;
    ...
    bindItemCardEvents(card, item) {
        ...
        card.addEventListener('mouseenter', () => {
            ...
            card.style.boxShadow = '0 20px 40px rgba(0, 0, 0, 0.4), 0 0 30px rgba(139, 95, 255, 0.2)';
    ...
    toggleHeart(itemId, button) {
        ...
        if (isHearted) {
            AppState.userProfile.heartedPosts = AppState.userProfile.heartedPosts.filter(id => id !== itemId);
    ...
    submitForm() {
        ...
        setTimeout(() => {
            const newItem = {
                ...
                originalPrice: parseInt(document.getElementById('itemOriginalPrice').value) || null,
                description: document.getElementById('itemDescription').value,
                hostel: document.getElementById('itemHostel').value,
                icon: this.getCategoryIcon(document.getElementById('itemCategory').value),
                isBoosted: false,
                hearts: 0,
                sellerId: 'user1'
            };
...
class Chat {
    ...
    openChat(chatId) {
        const conversation = AppState.chatData.conversations.find(c => c.id === chatId);
...
class Profile {
    ...
    updateStats() {
        ...
        if (totalTransactionsEl) {
            utils.animateValue(totalTransactionsEl, 0, AppState.userProfile.totalTransactions, 1000);
        }

        if (moneySavedEl) {
            utils.animateValue(moneySavedEl, 0, AppState.userProfile.moneySaved, 1000, 
                val => `₹${val}`);
        }
    }
    ...
    loadHeartedPosts() {
        ...
        AppState.userProfile.heartedPosts.forEach(itemId => {
            ...
            heartedCard.innerHTML = `
                <div class="hearted-icon" style="font-size: 2rem; margin-bottom: 8px;">${item.icon}</div>
                <h4 style="margin: 0 0 8px 0; color: #f5f5f5; font-size: 14px;">${item.title}</h4>
                <p style="margin: 0; color: #FF3366; font-weight: bold;">${utils.formatPrice(item.price)}</p>
            `;
...
function initModals() {
    ...
    document.addEventListener('click', (e) => {
        if (e.target.id === 'cancelBoost' || e.target.id === 'cancelRemove') {
